...
### Per-agent chat starters

New Session and empty Chat panes normally show four built-in starters. To replace
them for one agent, configure up to five ordered entries under `agents.entries.<id>.starters`:
...
```json5
{
  agents: {
    entries: {
      design: {
        starters: [
          { label: "Create concepts", prompt: "Start the concept workflow" },
          { label: "Review an image" },
        ],
      },
    },
  },
...
`label` is the button text. `prompt` is the message sent through the ordinary
chat send path and defaults to `label`. Missing, empty, oversized, or otherwise
invalid starter metadata leaves the four built-in starters in place.
