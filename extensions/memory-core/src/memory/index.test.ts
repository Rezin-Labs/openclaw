...
describe("memory index", () => {
  ...
  it("preserves fallback body boosts in hybrid ranking", async () => {
    const cfg = createCfg({
      minScore: 0,
      hybrid: { enabled: true, vectorWeight: 0, textWeight: 1 },
    });
    ...
    for (let index = 0; index < 5; index += 1) {
      const duplicateDir = path.join(memoryDir, `hybrid-alpha-${index}`);
      await fs.mkdir(duplicateDir, { recursive: true });
      await fs.writeFile(path.join(duplicateDir, "alpha.md"), "Unrelated path-only candidate.");
    }
    await fs.writeFile(path.join(memoryDir, "hybrid-body-match.md"), "Alpha");
    await manager.sync({ reason: "test" });

    const results = await manager.search("alpha gamma", { maxResults: 1, minScore: 0 });
    expect(results).toHaveLength(1);
    expect(results[0]?.path).toContain("memory/hybrid-body-match.md");
  });
