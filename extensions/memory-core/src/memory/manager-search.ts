...
export async function searchPathKeyword(params: {
  ...
  // Exact filenames need bounded headroom until body relevance and temporal
  // decay run in the manager; lexical candidates retain the normal query cap.
  return [...byId.values()]
    .toSorted(comparePathKeywordSearchResults)
    .slice(0, exactPathLimit + params.limit);
}
