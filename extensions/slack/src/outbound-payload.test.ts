...
describe("slackOutbound sendPayload", () => {
  ...
  it("posts Slack-safe text when a portable table cannot render natively", async () => {
    ...
    const client = createSlackSendTestClient();
    const cfg = { channels: { slack: { botToken: "xoxb-test" } } };
    const sendOptions: Array<NonNullable<Parameters<typeof sendMessageSlack>[2]>> = [];
    ...
    expect(sendOptions).not.toHaveLength(0);
    expect(sendOptions.every((opts) => opts.deliveryQueueId === undefined)).toBe(true);
    expect(sendOptions.every((opts) => opts.onPlatformSendDispatch === undefined)).toBe(true);
  ...
  it("keeps mixed segment order when Slack rejects the native chart", async () => {
    ...
    expect(
      (
        client.chat.postMessage.mock.calls[1]?.[0] as { blocks?: Array<{ type?: string }> }
    ...
    expect(
      (client.chat.postMessage.mock.calls[2]?.[0] as { blocks?: unknown }).blocks,
    ).toBeUndefined();
    expect(
      (
        client.chat.postMessage.mock.calls[3]?.[0] as { blocks?: Array<{ type?: string }> }
      ).blocks?.map((block) => block.type),
    ).toEqual(["actions"]);
  });
